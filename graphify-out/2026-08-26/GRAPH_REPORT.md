...
## Corpus Check
- 146 files · ~171,129 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 739 nodes · 1368 edges · 103 communities (75 shown, 28 thin omitted)
...
- `OmniRouteController` --inherits--> `Controller`  [EXTRACTED]
  app/Http/Controllers/OmniRouteController.php → app/Http/Controllers/Controller.php
...
Cohesion: 0.05
Nodes (25): AuthController, CartoesController, CategoriasController, Controller, ExportController, FinancasController, HomeController, TelegramWebhookController (+17 more)
...
### Community 5 - "User"
Cohesion: 0.15
Nodes (7): User, Illuminate\Database\Eloquent\Attributes\Fillable, Illuminate\Database\Eloquent\Attributes\Hidden, Illuminate\Database\Eloquent\Relations\HasMany, Illuminate\Foundation\Auth\User, Illuminate\Notifications\Notifiable, PhotoModuleTest
...
Cohesion: 0.08
Nodes (10): EstudosController, CartaoPrevisao, NotaFiscalItem, SalaryProfile, StudyGoal, StudyLog, WhatsappLog, Illuminate\Database\Eloquent\Factories\HasFactory (+2 more)
...
- **Why does `User` connect `User` to `PhotoController`, `Illuminate\Http\Request`, `DatabaseSeeder.php`, `OmniRouteModuleTest`, `ImagineModuleTest`, `Illuminate\Database\Eloquent\Model`, `SalaryProjectionTest.php`, `FiscalModuleTest`, `TestCase`, `FFmpegModuleTest`, `FiscalNoticia`?**
  _High betweenness centrality (0.098) - this node is a cross-community bridge._
- **Why does `FiscalConcurso` connect `FiscalConcurso` to `Illuminate\Http\Request`, `User`, `FiscalConcursosController`, `Illuminate\Database\Eloquent\Model`, `FiscalConcursoDataService`, `FiscalNewsAiService`, `FiscalNewsCrawlerService`, `FiscalModuleTest`, `FiscalNoticia`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `Controller` connect `Illuminate\Http\Request` to `PhotoController`, `.http`, `ImagineController`, `FFmpegController`, `FiscalConcursosController`, `Illuminate\Database\Eloquent\Model`, `SalaryController`?**
